/**
 * Script PRODUCTION pour supprimer TOUS les produits/packs de démonstration
 * ⚠️ NE SUPPRIME PAS LES PHOTOS - Seulement les entrées en base de données
 * 
 * Usage:
 *   Dry-run: npx tsx scripts/remove-demo-content-production.ts
 *   Exécution: npx tsx scripts/remove-demo-content-production.ts --execute
 */
...
// Patterns pour identifier les produits/packs de démonstration
const demoPatterns = [
  /^test/i,
  /^demo/i,
  /^sample/i,
...
// Noms exacts de produits demo connus
const demoProductNames = [
  ...
  'Boucles Créoles Berbères',
]

async function main() {
  console.log('🔍 Suppression du contenu de démonstration (PRODUCTION)\n')
  console.log('='.repeat(60) + '\n')

  if (isDryRun) {
    console.log('⚠️  MODE DRY-RUN (aucune modification)\n')
    console.log('   Pour exécuter: npx tsx scripts/remove-demo-content-production.ts --execute\n')
  } else {
    console.log('🚨 MODE EXÉCUTION - Suppression définitive du contenu demo\n')
  }
  ...
  try {
    ...
    console.log('ID  | Nom                              | Catégorie')
    console.log('-'.repeat(60))
    
    demoProducts.forEach(p => {
      const name = p.name.padEnd(30)
      const category = (p.category || 'N/A').padEnd(15)
      console.log(`${String(p.id).padStart(3)} | ${name} | ${category}`)
    })
