...
import { sendInvoiceEmailBodySchema, validateWithSchema } from '@/lib/validations'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = validateWithSchema(sendInvoiceEmailBodySchema, raw)
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides' }, { status: 422 })
    }
    const { order_id, customer_email, customer_name, invoice_data } = validation.data
    ...
    const emailContent = generateEmailContent({
      ...
      invoice_data: invoice_data as EmailData['invoice_data'],
    })
